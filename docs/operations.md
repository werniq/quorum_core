...
- `QUORUM_UI_AUTH_ENABLED` defaults to `true` (setup token + login). For local open UI, set `false` or use `QUORUM_DEMO_MODE=true` on a localhost bind. Supply `QUORUM_SETUP_TOKEN` (≥24 chars) for first admin; the admin password must be ≥12 characters and not a known default (`weak_password` otherwise).
